...
def make_speccube(simfile: str, spfile: str, size: float, npix=256, redshift=None, center=None, proj='z', zrange=None,
                  ...
                  seed=0, nosmooth=False, nh=None, progress=False, method='Tables'):
    """
    :param method:
    :param simfile: (str) Simulation file (Gadget)
    ...
    #else:
    #    print('HERE TOO')
    #    energy = np.linspace(2.0, 8.0, 9901)  # [keV]
    #    nene = len(energy)
    #    # TODO: include d_ene while generating the table
    #    d_ene = (energy[-1] - energy[0]) / (nene - 1)  # [keV] Assuming uniform energy interval.
    #    # Initializing spcube with double precision
    ...
    #    spcube = np.full((npix, npix, nene), 0., dtype=np.float64)
        ...
        # Cython loop for mapping
    #    make_speccube_loop_with_lib(spcube, iter_, x, y, hsml, norm, z_eff, temp_kev, method, metallicity, energy)
    ...
    if nh is not None:
        ...
        if method != 'Tables':
            result = absorption.convert_nh(result, nh)
    else:
        if 'nh' in spectable:
